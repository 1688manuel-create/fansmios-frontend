...
export default function PaymentModal({ 
  ...
  // Simulador de pago para el entorno de desarrollo (Bypass de Onramper para pruebas)
  ...
  const handleApplyCoupon = () => {
    ...
    // Aquí iría la lógica para validar contra el backend
    setCouponApplied(true);
  };

  // 🔥 MAGIA NEGRA: URL Dinámica de Onramper tematizada para que parezca tuya
  // Escondemos los logos y colores de Onramper usando sus parámetros de URL (Theme: dark, colores hexadecimales adaptados a FANSMIOS)
  const ONRAMPER_API_KEY = process.env.NEXT_PUBLIC_ONRAMPER_API_KEY || 'pk_test_x5M_5fdXtc1fOQm3Z0fSRVwAAc23B86iI-e6q1o70U00'; // Usa tu llave pública
  const finalPrice = amountUsd || price;
      ...
      <div className="bg-[#0a0a0a] border border-white/5 rounded-[2rem] p-6 md:p-8 max-w-4xl w-full shadow-[0_0_50px_rgba(0,0,0,0.8)] relative overflow-hidden flex flex-col md:flex-row gap-8 nm-inset">
        ...
        <div className="w-full md:w-1/2 flex flex-col pt-2">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-400 text-sm mt-2 font-medium">Estás a un paso de desbloquear contenido exclusivo de forma segura.</p>
          </div>

          {/* Ticket de Compra Neumórfico */}
          <div className="nm-inset border border-white/5 rounded-2xl p-6 mb-8 flex-1">
            <div className="flex justify-between items-center mb-4 text-gray-400 text-sm font-bold uppercase tracking-widest">
              <span>Subtotal</span>
              <span className="text-white">${price.toFixed(2)} USD</span>
            </div>
            <div className="flex justify-between items-center mb-4 text-gray-400 text-sm font-bold uppercase tracking-widest">
            ...
            <div className="flex justify-between items-end text-white">
              ...
              <span className="text-4xl font-black text-red-500 drop-shadow-[0_0_10px_rgba(239,68,68,0.3)]">
                ${couponApplied ? (price * 0.9).toFixed(2) : price.toFixed(2)} <span className="text-lg text-red-700">USD</span>
              </span>
            </div>
          </div>

          {/* Input de Cupones (Hundido) */}
        ...
        {/* ==============================================
            COLUMNA DERECHA: EL CAJERO (ONRAMPER IFRAME)
        ============================================== */}
        <div className="w-full md:w-1/2 flex flex-col relative">
          <div className="bg-[#0e0e0e] border border-white/5 rounded-[2rem] h-[500px] w-full overflow-hidden relative shadow-2xl">
            
            {/* Estado de Carga mientras se genera la Billetera */}
            {!payAddress ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center bg-nm-base z-10">
                <Loader2 className="w-10 h-10 text-red-500 animate-spin mb-4" />
                <p className="text-white font-bold text-lg">Generando pasarela segura...</p>
                <p className="text-gray-500 text-xs mt-2">Conectando con servidores bancarios.</p>
              </div>
            ) : (
              <>
                {/* 🛡️ EL IFRAME MÁGICO DE ONRAMPER */}
                <iframe 
          ...
          {/* BOTÓN MODO DESARROLLADOR (Para probar sin usar tarjeta real) */}
          {process.env.NODE_ENV === 'development' && (
            ...
            >
              {isProcessing ? (
                <><Loader2 className="w-4 h-4 animate-spin"/> Simulando Pago Bancario...</>
              ) : (
                <><ShieldCheck className="w-4 h-4" /> [DEV] Simular Pago Exitoso</>
              )}
            </button>
          )}
          ...
          <p className="text-[10px] text-gray-500 text-center mt-5 flex items-center justify-center gap-1.5 font-bold uppercase tracking-widest">
            <Lock className="w-3 h-3"/> Encriptación AES-256 de extremo a extremo
          </p>
        </div>
