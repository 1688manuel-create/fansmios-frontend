...
export default function Feed() {
  ...
  const handleDeleteComment = async (commentId: string) => {
    ...
    try { await api.delete(`/posts/comments/${commentId}`); fetchData(); } catch (error) { alert("Error al eliminar comentario."); }
  };
  ...
  const handleUnlockBundle = async (bundle: any) => {
    try {
      ...
      if (data.success || data.receipt) { alert('✅ ¡Paquete comprado con éxito por PayRam!'); fetchData(); } 
      else {
  ...
  const handleUnlockClick = async (post: any) => {
    try {
      ...
      if (data.success || data.receipt) { alert('✅ ¡Contenido desbloqueado con PayRam!'); fetchData(); } 
  ...
  const handlePublish = async () => {
    ...
    try {
      const formData = new FormData(); formData.append('content', newPostContent); formData.append('isPPV', String(isPPV)); formData.append('price', String(isPPV ? Number(price) : 0));
  ...
  const handleStartLive = async () => {
    ...
    if (!title) return; 
    try { const res = await liveService.createStream(title); router.push(`/live/${res.liveStream.id}`); } catch (error) { alert("Error al iniciar Live."); }
  };
  ...
  const handleDeleteStory = async (storyId: string) => {
    ...
    try { await api.delete(`/stories/${storyId}`); alert("✅ Historia eliminada."); setActiveStory(null); fetchData(); } 
  ...
  const handleDeletePost = async (postId: string) => {
    ...
    try { await api.delete(`/posts/${postId}`); alert("✅ Publicación eliminada."); fetchData(); } 
  ...
  const buildCommentTree = (comments: any[]) => {
    if (!comments) return [];
    const commentMap = new Map(); const roots: any[] = [];
                    ...
                    <div className="w-16 h-16 rounded-full p-1 transition-transform group-hover:scale-105 bg-gradient-to-tr from-yellow-400 to-yellow-600 shadow-[0_0_15px_rgba(234,179,8,0.3)] mt-1">
                      <div className="w-full h-full rounded-full bg-black border-2 border-black flex items-center justify-center overflow-hidden">
                        {vipCreator.creatorProfile?.profileImage ? (
                          <img src={getImageUrl(vipCreator.creatorProfile.profileImage)} className="w-full h-full object-cover object-center" alt="Avatar VIP" />
                        ) : (
              ...
              {stories.map(story => (
                <div key={story.id} onClick={() => openStory(story)} className="flex flex-col items-center gap-1 cursor-pointer group shrink-0">
                  <div className="w-16 h-16 rounded-full p-1 transition-transform group-hover:scale-105 bg-gradient-to-tr from-red-600 to-orange-500 shadow-lg">
                    <div className="w-full h-full rounded-full bg-black border-2 border-black flex items-center justify-center overflow-hidden">
                      {story.creator?.creatorProfile?.profileImage ? <img src={getImageUrl(story.creator.creatorProfile.profileImage)} alt="Avatar" className="w-full h-full object-cover object-center" /> : <span className="text-xl text-white font-bold">{(story.creator?.username || 'U')[0].toUpperCase()}</span>}
                    </div>
                  </div>
                  ...
                  <div key={stream.id} onClick={() => router.push(`/live/${stream.id}`)} className="relative shrink-0 w-56 h-32 rounded-2xl overflow-hidden cursor-pointer group border border-red-500/30 shadow-lg">
                    <div className="absolute inset-0 bg-black">
                      {stream.creator?.creatorProfile?.profileImage ? (
                        <img src={getImageUrl(stream.creator.creatorProfile.profileImage)} className="w-full h-full object-cover object-center opacity-40 group-hover:opacity-60 transition-opacity" alt="Fondo" />
                  ...
                  return (
                    <React.Fragment key={`${post.id}-${index}`}>
                      ...
                      <div id={`post-${post.id}`} className={`scroll-mt-24 transition-all duration-500 p-4 sm:p-6 rounded-[2rem] space-y-4 relative overflow-hidden shadow-xl border ${post.isPromoted ? 'bg-[#111] border-yellow-500/30' : 'bg-[#0a0a0a] border-white/5'}`}>
                        ...
                        {isOwner && (
                          <button onClick={() => handleDeletePost(post.id)} className="absolute top-6 right-6 text-gray-500 hover:text-red-500 hover:bg-red-500/10 p-2.5 rounded-full transition-all z-20" title="Eliminar publicación"><Trash2 className="w-5 h-5" /></button>
                        )}
                        ...
                        <div className="flex justify-between items-center relative z-10">
                          <div className="flex items-center gap-3">
                            <div onClick={() => router.push(`/${post.user.username}`)} className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold shadow-lg overflow-hidden cursor-pointer border ${post.isPromoted ? 'border-yellow-500' : 'border-white/10'}`}>
                              {post.user?.creatorProfile?.profileImage ? <img src={getImageUrl(post.user.creatorProfile.profileImage)} className="w-full h-full object-cover object-center" alt="Autor" /> : <div className={`w-full h-full flex items-center justify-center ${post.isPromoted ? 'bg-gradient-to-r from-yellow-500 to-yellow-600' : 'bg-gradient-to-r from-red-500 to-orange-500'}`}>{(post.user?.username || 'U')[0].toUpperCase()}</div>}
                            </div>
                            ...
                            {post.mediaUrl && (
                              <div className="mt-4 rounded-2xl overflow-hidden nm-inset border border-white/5 relative bg-black/50 flex justify-center">
                                {post.mediaUrl.match(/\.(mp4|mov|webm)$/i) ? (
                                  <video controls controlsList="nodownload" src={getImageUrl(post.mediaUrl)} className="w-full max-h-[600px] object-cover object-center" />
                                ) : (
                                  <img src={getImageUrl(post.mediaUrl, post.user?.username)} className="w-full h-auto object-contain max-h-[600px] cursor-pointer" onClick={() => setExpandedImage({ url: getImageUrl(post.mediaUrl, post.user?.username), username: post.user?.username })} alt="Contenido del Post" />
                                )}
                              </div>
                            )}
                                ...
                                <div className="flex items-center gap-3">
                                  <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-3 py-1.5 overflow-hidden">
                                    {['❤️', '❤️‍🔥', '🤤', '🫦'].map((emoji) => {
                                      ...
                                      return (
                                        <button key={emoji} onClick={() => handleReact(post.id, emoji)} className={`flex items-center gap-1 transition-all duration-300 hover:scale-110 hover:grayscale-0 hover:opacity-100 ${isSelected ? 'scale-110 opacity-100 grayscale-0 drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]' : 'opacity-40 grayscale'}`} title="Reaccionar">
                                  ...
                                  <button onClick={() => setCommentingPostId(commentingPostId === post.id ? null : post.id)} className={`flex items-center gap-1.5 font-bold transition-all px-4 py-2.5 rounded-full bg-white/5 border border-white/10 ${commentingPostId === post.id ? 'text-blue-500 border-blue-500/30' : 'text-gray-400 hover:text-blue-400 hover:border-white/20'}`}><MessageCircle className="w-4 h-4" /><span className="text-sm">{totalComments}</span></button>
                                </div>
                                {!isOwner && <button onClick={() => { setTipRecipient(post.user); setIsTipModalOpen(true); }} className="flex items-center gap-1.5 text-gray-400 hover:text-green-500 font-bold transition-colors"><Coins className="w-5 h-5" /><span className="text-sm hidden sm:inline">Propina</span></button>}
                              </div>

                              {commentingPostId === post.id && (
                                <div className="flex flex-col gap-2 animate-fade-in mt-2">
                                  {replyingToCommentId && <div className="flex justify-between items-center bg-blue-900/20 px-3 py-1 text-xs text-blue-400 rounded-lg"><span>Respondiendo al comentario...</span><button onClick={() => setReplyingToCommentId(null)}><X className="w-3 h-3"/></button></div>}
                                  <div className="flex gap-2"><input type="text" value={commentText} onChange={(e) => setCommentText(e.target.value)} placeholder="Escribe un comentario..." className="flex-1 bg-black/50 border border-white/10 rounded-full px-5 py-2.5 text-sm text-white outline-none focus:border-blue-500/50" onKeyDown={(e) => e.key === 'Enter' && submitComment(post.id)}/><button onClick={() => submitComment(post.id)} disabled={isSubmittingComment || !commentText.trim()} className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center text-white hover:bg-blue-500 transition-colors shrink-0 disabled:opacity-50"><Send className="w-4 h-4" /></button></div>
                                </div>
                              )}
          ...
          <aside className="hidden lg:block w-80 shrink-0">
            <div className="sticky top-24 space-y-6">
              <div className="bg-[#0a0a0a] rounded-[2rem] border border-yellow-500/20 shadow-xl overflow-hidden">
                <div className="bg-gradient-to-r from-yellow-900/30 to-black p-5 border-b border-yellow-500/20 flex items-center gap-2"><TrendingUp className="w-5 h-5 text-yellow-500" /><h3 className="text-white font-black uppercase tracking-widest text-sm">Trending VIP</h3></div>
                <div className="p-2">
                  {trendingCreators.map((creator, idx) => (
                    <div key={creator.id} onClick={() => router.push(`/${creator.username}`)} className="flex items-center justify-between p-3 rounded-2xl hover:bg-white/5 cursor-pointer group">
                      <div className="flex items-center gap-3"><div className="w-12 h-12 rounded-full bg-gradient-to-tr from-yellow-400 to-yellow-600 flex items-center justify-center text-black font-bold border border-yellow-500/50 overflow-hidden shadow-lg">{creator.creatorProfile?.profileImage ? <img src={getImageUrl(creator.creatorProfile.profileImage)} className="w-full h-full object-cover object-center" alt="Trending" /> : (creator.username || 'U')[0].toUpperCase()}</div><div><p className="text-white font-bold text-sm flex items-center gap-1">{creator.username || 'Usuario'} {idx === 0 && <Star className="w-3 h-3 text-yellow-500 fill-yellow-500"/>}</p></div></div><ChevronRight className="w-4 h-4 text-gray-600 group-hover:text-yellow-500 transition-transform group-hover:translate-x-1" />
                    </div>
                  ))}
                </div>
                {(user?.role === 'CREATOR' || user?.role === 'ADMIN') && <div className="p-4 border-t border-white/5 text-center"><p onClick={() => setIsBoostModalOpen(true)} className="text-gray-500 text-[10px] uppercase font-bold tracking-widest cursor-pointer hover:text-yellow-500">¿Quieres aparecer aquí?</p></div>}
              </div>
              ...
              {featuredBundle && (
                <div className="bg-gradient-to-br from-blue-900/40 to-purple-900/20 rounded-[2rem] border border-blue-500/30 shadow-2xl p-6 relative overflow-hidden group">
                  <div className="absolute top-0 right-0 bg-blue-500 text-white text-[10px] font-bold px-3 py-1 rounded-bl-xl z-10 flex items-center gap-1 shadow-lg"><Zap className="w-3 h-3 fill-white"/> RECOMENDADO</div>
                  <div onClick={() => router.push(`/${featuredBundle.creator?.username}`)} className="cursor-pointer flex flex-col items-center text-center">
                    <div className="w-16 h-16 rounded-full border-2 border-blue-500 p-1 mb-3 relative group-hover:scale-105 transition-transform shadow-[0_0_15px_rgba(59,130,246,0.4)]"><div className="w-full h-full rounded-full overflow-hidden bg-black">{featuredBundle.creator?.creatorProfile?.profileImage ? (<img src={getImageUrl(featuredBundle.creator.creatorProfile.profileImage)} className="w-full h-full object-cover object-center" alt="Avatar Bundle" />) : (<span className="w-full h-full flex items-center justify-center text-xl font-bold text-white bg-gradient-to-r from-blue-500 to-purple-500">{(featuredBundle.creator?.username || 'U')[0].toUpperCase()}</span>)}</div></div>
                    <h3 className="text-white font-black text-lg leading-tight">@{featuredBundle.creator?.username}</h3><p className="text-blue-300 text-[11px] font-bold mt-1 mb-4 bg-blue-500/10 px-3 py-1 rounded-full border border-blue-500/20">Paquete: {featuredBundle.title}</p>
                  </div>
                  <div className="flex flex-col gap-2 relative z-10"><button onClick={(e) => { e.stopPropagation(); handleUnlockBundle(featuredBundle); }} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-2.5 rounded-xl transition-all shadow-[0_0_15px_rgba(59,130,246,0.5)] flex items-center justify-center gap-2 text-sm"><Unlock className="w-4 h-4"/> Comprar Paquete ${featuredBundle.price}</button><button onClick={(e) => { e.stopPropagation(); router.push(`/${featuredBundle.creator?.username}`); }} className="w-full bg-transparent border border-blue-500/50 hover:bg-blue-500/10 text-blue-400 font-bold py-2.5 rounded-xl transition-all flex items-center justify-center gap-2 text-sm"><Crown className="w-4 h-4"/> Ver Perfil y Suscribirse</button></div>
                </div>
              )}
            </div>
          </aside>
        ...
        {activeStory && (
          <div className="fixed inset-0 z-[200] bg-black/95 flex flex-col animate-fade-in select-none">
            <div className="flex justify-between items-center p-4 absolute top-0 w-full z-10 bg-gradient-to-b from-black/80 to-transparent">
              <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full overflow-hidden bg-black border border-white/20 shadow-lg">{activeStory.creator?.creatorProfile?.profileImage ? <img src={getImageUrl(activeStory.creator.creatorProfile.profileImage)} className="w-full h-full object-cover object-center" /> : <div className="w-full h-full flex items-center justify-center text-white font-bold">{(activeStory.creator?.username || 'U')[0].toUpperCase()}</div>}</div><span className="text-white font-bold drop-shadow-md">@{activeStory.creator?.username}</span></div>
              <div className="flex items-center gap-3">{user?.id === activeStory.creator?.id && (<button onClick={() => handleDeleteStory(activeStory.id)} className="w-10 h-10 bg-black/50 hover:bg-red-600 text-gray-300 hover:text-white rounded-full flex items-center justify-center backdrop-blur-md border border-white/10 transition-all shadow-lg" title="Eliminar historia"><Trash2 className="w-5 h-5"/></button>)}<button onClick={() => setActiveStory(null)} className="w-10 h-10 bg-black/50 hover:bg-white/20 text-gray-300 hover:text-white rounded-full flex items-center justify-center backdrop-blur-md border border-white/10 transition-all shadow-lg"><X className="w-6 h-6"/></button></div>
            </div>
            <div className="flex-1 flex justify-center items-center p-4 pt-20">{activeStory.mediaUrl?.match(/\.(mp4|mov|webm)$/i) ? (<video src={getImageUrl(activeStory.mediaUrl)} autoPlay controls className="max-w-full max-h-full rounded-2xl shadow-2xl object-contain" />) : (<img src={getImageUrl(activeStory.mediaUrl, activeStory.creator?.username)} className="max-w-full max-h-[85vh] rounded-2xl shadow-2xl object-contain pointer-events-none" />)}</div>
          </div>
        )}

        {isTipModalOpen && tipRecipient && (<TipModal creatorName={tipRecipient.username} onClose={() => setIsTipModalOpen(false)} onContinue={async (amount, message) => { setIsTipModalOpen(false); try { const data = await paymentService.createPaymentIntent({ amount: amount || 0, type: 'TIP', creatorId: tipRecipient.id, description: `Propina: ${message}` }); if (data.success || data.receipt) { alert('✅ ¡Propina enviada con PayRam!'); fetchData(); } else { setClientSecret(data.clientSecret); setSelectedPost({ id: 'tip', price: amount }); setIsPaymentModalOpen(true); } } catch (error) { alert('Error al procesar propina.'); } }} />)}
        {isPaymentModalOpen && clientSecret && selectedPost && (<PaymentModal clientSecret={clientSecret} price={selectedPost.price || 0} creatorId={selectedPost.user?.id || tipRecipient?.id || featuredBundle?.creatorId} onClose={() => setIsPaymentModalOpen(false)} onSuccess={async () => { setIsPaymentModalOpen(false); if (selectedPost.isBundle) { try { await api.post('/bundles/purchase', { bundleId: selectedPost.id }); } catch (e) { console.error('Error interno al liberar el paquete:', e); } } alert("¡Pago exitoso! El contenido ya es tuyo 🔓"); fetchData(); }} />)}
        {isBoostModalOpen && (<BoostModal onClose={() => setIsBoostModalOpen(false)} creatorBalance={walletBalance} />)}
        {expandedImage && (<div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/95 backdrop-blur-md p-4 animate-fade-in cursor-zoom-out select-none" onClick={() => setExpandedImage(null)} onContextMenu={(e) => e.preventDefault()}><button onClick={() => setExpandedImage(null)} className="absolute top-6 right-6 text-gray-300 hover:text-white bg-white/5 hover:bg-white/10 p-3 rounded-full transition-all z-50 border border-white/10" title="Cerrar"><X className="w-6 h-6" /></button><div className="relative inline-block" onClick={(e) => e.stopPropagation()}><img src={expandedImage.url} alt="Exclusivo" className="max-w-full max-h-[90vh] object-contain rounded-lg shadow-[0_0_50px_rgba(0,0,0,0.8)] cursor-default select-none pointer-events-none" draggable="false" /><div className="absolute inset-0 z-10 w-full h-full cursor-default"></div></div></div>)}
