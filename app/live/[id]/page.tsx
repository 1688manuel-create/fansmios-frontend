...
export default function LiveRoom() {
  ...
  // 2. ESTADOS DE CHAT Y GAMIFICACIÓN
  ...
  // 🛡️ VERIFICACIÓN DE USUARIO Y CARGA DE DATOS
  useEffect(() => {
    try {
  ...
  const loadStreamData = async () => {
    try {
      ...
      if (data.hasAccess) {
        ...
        setMessages([{ isSystem: true, content: `👋 Conexión segura Covra Pay activa para @${currentUser.username}.` }, ...(data.stream.messages || [])].slice(-100));
  ...
  const triggerHeart = () => {
    ...
    el.innerHTML = '❤️';
    el.className = 'absolute bottom-0 text-red-500 animate-float text-3xl drop-shadow-md';
    el.style.left = `${Math.floor(Math.random() * 80) + 10}%`;
    heartsContainerRef.current.appendChild(el);
    setTimeout(() => el.remove(), 2500); 
  };

  // 📡 INYECCIÓN DEL CEREBRO DE COMUNICACIONES (HOOK CUSTOM)
  ...
  const sendGift = async (gift: Gift) => {
    ...
    try {
      ...
      const fakeMsg = { content: `ha enviado un ${gift.emoji} ${gift.name}`, isDonation: true, amount: gift.amount, user: user, streamId: id };
      setMessages((prev) => [...prev.slice(-99), fakeMsg]);
  ...
  const handleKickUser = async (targetUserId: string, targetUsername: string) => {
    if (targetUserId === user.id) return; // No te puedes expulsar a ti mismo
    if (!window.confirm(`🚨 ¿Deseas EXPULSAR a @${targetUsername} de la transmisión?`)) return;

    try {
      // Emitimos al socket para que el servidor lo saque en tiempo real
      socketRef.current?.emit('kickParticipant', { 
        streamId: id, 
        userId: targetUserId,
        username: targetUsername 
      });
      alert(`🚫 Usuario @${targetUsername} expulsado.`);
    } catch (error) {
      alert("Error al intentar expulsar al usuario.");
    }
  };
  ...
  // 🎨 RENDERIZADO DEL ORQUESTADOR
  return (
    <div className="fixed inset-0 bg-black text-white font-sans overflow-hidden">
      
      {/* 🎬 VIDEO LAYER */}
      <div className="absolute inset-0 z-0 bg-[#050505] [&_video]:!object-cover md:[&_video]:!object-contain [&_video]:!w-full [&_video]:!h-full" onContextMenu={(e) => e.preventDefault()}>
      ...
      {/* 🛑 PAYWALL LAYER */}
      {!hasAccess && (
        <PaywallLayer price={streamData.price} isProcessing={isProcessing} onBuy={handleBuyTicket} />
      )}

      {/* 📺 PREPARATION LAYER (Solo Creador) */}
      {isCreatorOrAdmin && !isLiveActive && hasAccess && (
        <PreparationLayer onStart={() => setIsLiveActive(true)} />
      )}

      {/* 💎 UI INMERSIVA (MÓDULOS TÁCTICOS) */}
      {hasAccess && (
        <div className="absolute inset-0 z-10 flex flex-col justify-between pointer-events-none">
          ...
          <TopHUD 
            ...
            viewersCount={connectedUsers.length > 0 ? connectedUsers.length : viewersCount} 
            uptime={uptime} 
            onShowViewers={() => setShowViewersModal(true)} 
            onExit={isCreatorOrAdmin ? handleEndStream : () => router.push('/explore')} 
          />

          <MiddleHUD topDonators={topDonators} streak={streak} />
          ...
          <LiveChat 
            ...
            onKick={handleKickUser} // 👈 NUEVO
            currentUser={user} // 👈 NUEVO
            isCreatorOrAdmin={isCreatorOrAdmin} // 👈 NUEVO 
          />
        </div>
      )}
      ...
      {/* 👁️ VIEWERS MODAL */}
      {showViewersModal && (
        <ViewersModal connectedUsers={connectedUsers} onClose={() => setShowViewersModal(false)} />
      )}
    </div>
  );
}
...
// ============================================================================
// 🧩 SUB-COMPONENTES TÁCTICOS (LOS "ÓRGANOS" SEPARADOS)
// ============================================================================

// 📡 EL CEREBRO DE COMUNICACIONES
...
// 🔝 RADAR SUPERIOR
function TopHUD({ streamData, isCreatorOrAdmin, viewersCount, uptime, onShowViewers, onExit }: any) {
  return (
    <div className="p-4 flex justify-between items-start pointer-events-auto">
      ...
      <div className="flex flex-col items-end gap-2">
        <div className="flex items-center gap-2">
          <button onClick={onShowViewers} className="flex items-center gap-1.5 bg-black/40 hover:bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10 text-xs font-bold transition-colors cursor-pointer shadow-lg">
            <Eye className="w-3.5 h-3.5 text-green-400" /> {viewersCount}
          </button>
          {!isCreatorOrAdmin ? (
            <button onClick={onExit} className="w-8 h-8 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center hover:bg-white/20 transition-colors">
              <X className="w-4 h-4" />
            </button>
          ) : (
            <button onClick={onExit} className="bg-red-600/90 backdrop-blur-md hover:bg-red-600 font-black px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-lg text-xs">
...
function MiddleHUD({ topDonators, streak }: { topDonators: Donator[], streak: number }) {
  return (
...
function LiveChat({ messages, chatInput, setChatInput, handleSendMessage, handleSendHeart, showGiftMenu, setShowGiftMenu, sendGift, chatEndRef, heartsContainerRef, onKick, currentUser, isCreatorOrAdmin }: any) {
  return (
    <div className="w-full md:w-[450px] bg-gradient-to-t from-black/90 via-black/40 to-transparent pt-12 pb-4 px-4 flex flex-col justify-end pointer-events-auto mt-auto relative">
      <div ref={heartsContainerRef} className="absolute bottom-16 right-4 w-16 h-64 pointer-events-none overflow-visible z-40" />
      
      <div className="max-h-[30vh] overflow-y-auto flex flex-col gap-2 custom-scrollbar pb-2" style={{ WebkitMaskImage: 'linear-gradient(to bottom, transparent, black 15%, black 100%)' }}>
        {messages.map((msg: any, i: number) => {
          ...
          const canModerate = isCreatorOrAdmin && msg.user?.id !== currentUser?.id;

          return (
            <div key={i} className={`text-sm px-3 py-1.5 rounded-2xl w-fit max-w-[90%] group/msg ${msg.isDonation ? 'bg-gradient-to-r from-green-500/20 to-transparent border-l-2 border-green-500' : 'bg-black/30 backdrop-blur-sm'}`}>
              <div className="flex items-center gap-2">
                <span 
                  onClick={() => canModerate && onKick(msg.user?.id, msg.user?.username)}
                  className={`font-bold text-xs cursor-pointer hover:underline ${msg.user?.role === 'ADMIN' ? 'text-red-400' : 'text-gray-300'} ${canModerate ? 'hover:text-red-500' : ''}`}
                >
                  {msg.user?.username || 'Usuario'}:
                </span>
                {canModerate && (
                  <button onClick={() => onKick(msg.user?.id, msg.user?.username)} className="opacity-0 group-hover/msg:opacity-100 transition-opacity text-[10px] bg-red-600/20 text-red-500 px-1.5 py-0.5 rounded font-black uppercase">BAN</button>
                )}
              </div>
      ...
      {/* ... Resto del componente (GiftMenu y Inputs) se mantiene igual ... */}
      {showGiftMenu && (
      ...
      <div className="flex gap-2 items-center mt-2 relative z-30">
        <div className="flex-1 bg-black/50 backdrop-blur-md border border-white/20 rounded-full flex items-center px-4 py-1.5 focus-within:border-green-500 transition-colors">
          <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="Di algo cool..." className="bg-transparent text-white text-sm w-full outline-none py-1 placeholder-gray-400" />
          <button onClick={handleSendMessage} className="text-gray-400 hover:text-green-400 transition-colors p-1"><Send className="w-4 h-4" /></button>
        </div>
        <button onClick={handleSendHeart} className="w-10 h-10 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center text-white hover:bg-white/20 hover:text-red-500 transition-colors active:scale-90"><Heart className="w-5 h-5" /></button>
        <button onClick={() => setShowGiftMenu(!showGiftMenu)} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${showGiftMenu ? 'bg-green-500 text-black shadow-[0_0_15px_rgba(34,197,94,0.5)]' : 'bg-gradient-to-tr from-green-600 to-emerald-400 text-black hover:scale-105'}`}><Diamond className="w-5 h-5 fill-current" /></button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="absolute inset-0 z-40 bg-black/80 backdrop-blur-md flex flex-col items-center justify-center p-4">
      <div className="text-center max-w-sm px-6">
        <div className="w-24 h-24 bg-green-500/20 rounded-full flex items-center justify-center mx-auto mb-6 border border-green-500/30"><Tv className="w-10 h-10 text-green-500" /></div>
        ...
        <button onClick={onStart} className="w-full bg-green-500 hover:bg-green-400 text-black font-black text-xl py-4 rounded-full shadow-[0_0_30px_rgba(34,197,94,0.4)] flex items-center justify-center gap-2 transition-transform hover:scale-105"><Play className="w-6 h-6 fill-black" /> Iniciar Transmisión</button>
      </div>
    </div>
  );
}
    ...
    <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-xl flex items-center justify-center p-4">
      <div className="text-center p-8 bg-[#111]/90 rounded-3xl border border-white/10 shadow-[0_0_60px_rgba(0,0,0,0.8)] max-w-sm w-full">
        ...
        <div className="bg-black/50 p-4 rounded-xl mb-6 border border-white/5">
          ...
          <div className="text-5xl font-black text-green-500 font-mono tracking-tight">${price} <span className="text-base text-gray-500">USD</span></div>
        </div>
        <button onClick={onBuy} disabled={isProcessing} className="w-full bg-white text-black font-black py-4 rounded-full text-lg hover:scale-105 transition-transform disabled:opacity-50 flex items-center justify-center gap-2">
    ...
    <div className="absolute inset-0 z-[200000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 pointer-events-auto">
      <div className="bg-[#111] border border-white/10 rounded-3xl w-full max-w-sm overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.8)] animate-fade-in">
        <div className="flex items-center justify-between p-5 border-b border-white/10 bg-gradient-to-b from-white/5 to-transparent">
          <h3 className="text-white font-black text-lg flex items-center gap-2"><Eye className="w-5 h-5 text-green-400" /> Espectadores ({connectedUsers.length})</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-white p-1.5 bg-white/5 hover:bg-red-500 rounded-full transition-colors"><X className="w-5 h-5" /></button>
        </div>
            ...
            connectedUsers.map((p, i) => {
              ...
              return (
                <div key={p.identity || i} className="flex items-center gap-3 p-3 hover:bg-white/5 rounded-2xl transition-all cursor-default">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-green-500 to-blue-600 flex items-center justify-center font-black text-white shadow-lg border border-white/20">{displayName.charAt(0).toUpperCase()}</div>
                  <div className="flex flex-col">
                    <span className="text-sm font-bold text-white">{displayName}</span>
                    <span className="text-[10px] text-green-400 font-mono flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse"></span> Conectado</span>
                  </div>
                </div>
              );
            })
...
function StreamStage({ hasControl }: { hasControl: boolean }) {
  ...
  return (
    <div className="w-full h-full flex flex-col relative bg-transparent">
      <div className="absolute inset-0 z-0"><GridLayout tracks={mainTrack} style={{ width: '100%', height: '100%' }}><ParticipantTile /></GridLayout></div>
      {coHostTracks.length > 0 && (
        <div className="absolute top-20 right-4 flex flex-col gap-2 z-10 w-24">
          {coHostTracks.map((track, i) => (
            <div key={i} className="w-24 h-32 rounded-xl overflow-hidden border-2 border-green-500/50 bg-black/50 backdrop-blur-md shadow-lg"><ParticipantTile trackRef={track} /></div>
          ))}
        </div>
      )}
      {hasControl && (
