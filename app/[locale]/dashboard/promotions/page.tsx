...
export default function PromotionsDashboard() {
  ...
  const handleBuyBoost = async (packageId: string, price: number) => {
    if (walletBalance < price) {
      alert(`⚠️ Saldo insuficiente. Tienes $${walletBalance.toFixed(2)} y necesitas $${price}. Ve a tu Billetera para recargar o espera a generar más ventas.`);
      return;
    }

    const confirmMsg = `¿Estás seguro de invertir $${price} de tu saldo para comprar este Paquete VIP?`;
    ...
    try {
      ...
      alert(error.response?.data?.error || "Error procesando la compra.");
    } finally {
      setIsBuying(false);
    }
  };
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-8 animate-fade-in pb-24 relative">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="nm-inset px-6 py-3 rounded-2xl border border-white/5 flex items-center gap-4 bg-black/40">
            ...
            <div>
              <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Saldo Disponible</p>
          ...
          <div className="nm-inset border border-yellow-500/30 p-6 rounded-3xl bg-gradient-to-r from-yellow-900/20 to-transparent relative overflow-hidden">
            <div className="flex items-center justify-between relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black text-white">¡Eres VIP Nivel {activePromo.package}!</h3>
                  <p className="text-sm text-yellow-200">Tus publicaciones están apareciendo primero a todos los fans.</p>
                </div>
              </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 pt-4">
          ...
          <div className="nm-btn p-8 rounded-[2rem] border border-white/5 hover:border-blue-500/30 transition-all flex flex-col justify-between group relative overflow-hidden">
            ...
            <div>
              ...
              <h3 className="text-2xl font-black text-white mb-2">Fuego 24h</h3>
              <p className="text-gray-400 text-sm font-medium mb-6">Ideal para probar el agua y conseguir tus primeras suscripciones rápidas.</p>
              
              <ul className="space-y-3 mb-8">
                <li className="flex items-center gap-2 text-sm text-gray-300"><CheckCircle2 className="w-4 h-4 text-blue-500"/> Prioridad en Explorar</li>
                <li className="flex items-center gap-2 text-sm text-gray-300"><CheckCircle2 className="w-4 h-4 text-blue-500"/> Borde Neón Azul</li>
                <li className="flex items-center gap-2 text-sm text-gray-300"><CheckCircle2 className="w-4 h-4 text-blue-500"/> Duración: 24 Horas</li>
              </ul>
            </div>
          ...
          <div className="nm-inset p-8 rounded-[2rem] border border-purple-500/50 shadow-[0_0_30px_rgba(168,85,247,0.15)] flex flex-col justify-between transform md:-translate-y-4 relative overflow-hidden">
            <div className="absolute top-0 right-0 bg-purple-500 text-white text-[10px] font-black uppercase tracking-widest px-8 py-1 rotate-45 translate-x-6 translate-y-3 shadow-lg">Más Popular</div>
            <div>
              ...
              <h3 className="text-2xl font-black text-white mb-2">Cohete 48h</h3>
              <p className="text-gray-400 text-sm font-medium mb-6">El favorito de los creadores Top. Domina el algoritmo durante todo el fin de semana.</p>
              
              <ul className="space-y-3 mb-8">
                <li className="flex items-center gap-2 text-sm text-gray-200"><CheckCircle2 className="w-4 h-4 text-purple-400"/> Ranking VIP Superior</li>
                <li className="flex items-center gap-2 text-sm text-gray-200"><CheckCircle2 className="w-4 h-4 text-purple-400"/> Borde Neón Púrpura</li>
                <li className="flex items-center gap-2 text-sm text-gray-200"><CheckCircle2 className="w-4 h-4 text-purple-400"/> Notificación a usuarios cercanos</li>
                <li className="flex items-center gap-2 text-sm text-gray-200"><CheckCircle2 className="w-4 h-4 text-purple-400"/> Duración: 48 Horas</li>
              </ul>
            </div>
          ...
          <div className="nm-btn p-8 rounded-[2rem] border border-white/5 hover:border-yellow-500/30 transition-all flex flex-col justify-between group relative overflow-hidden">
            ...
            <div>
              ...
              <h3 className="text-2xl font-black text-white mb-2">Modo Dios 72h</h3>
              <p className="text-gray-400 text-sm font-medium mb-6">Acapara absolutamente todo. Quien entre a la plataforma te verá primero.</p>
              
              <ul className="space-y-3 mb-8">
                <li className="flex items-center gap-2 text-sm text-gray-300"><CheckCircle2 className="w-4 h-4 text-yellow-500"/> Posición #1 Garantizada</li>
                <li className="flex items-center gap-2 text-sm text-gray-300"><CheckCircle2 className="w-4 h-4 text-yellow-500"/> Corona Dorada en Perfil</li>
                <li className="flex items-center gap-2 text-sm text-gray-300"><CheckCircle2 className="w-4 h-4 text-yellow-500"/> Historias sugeridas</li>
                <li className="flex items-center gap-2 text-sm text-gray-300"><CheckCircle2 className="w-4 h-4 text-yellow-500"/> Duración: 3 Días Completos</li>
              </ul>
            </div>
