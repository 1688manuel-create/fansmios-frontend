          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="nm-inset p-6 rounded-[2rem] border border-green-500/30 relative overflow-hidden group">
                ...
                <h3 className="text-green-400 text-[10px] font-black uppercase tracking-widest mb-2 relative z-10">{t('lbl_income_today')}</h3>
                <p className="text-4xl font-black text-white relative z-10 drop-shadow-[0_0_10px_rgba(34,197,94,0.3)]">${financialStats.dailyIncome.toFixed(2)}</p>
              </div>
              ...
              <div className="nm-btn border border-white/5 p-6 rounded-[2rem] relative overflow-hidden group cursor-default">
                ...
                <h3 className="text-gray-500 text-[10px] font-black uppercase tracking-widest mb-2 relative z-10">{t('lbl_income_month')}</h3>
                <p className="text-4xl font-black text-white relative z-10">${financialStats.monthlyIncome.toFixed(2)}</p>
              </div>
              ...
              <div className="nm-btn border border-white/5 p-6 rounded-[2rem] relative overflow-hidden group cursor-default">
                ...
                <h3 className="text-gray-500 text-[10px] font-black uppercase tracking-widest mb-2 relative z-10">{t('lbl_conversion')}</h3>
                <p className="text-4xl font-black text-white relative z-10">{financialStats.conversionRate}</p>
              </div>
              ...
              <div className="nm-btn border border-white/5 p-6 rounded-[2rem] relative overflow-hidden group cursor-default">
                ...
                <h3 className="text-gray-500 text-[10px] font-black uppercase tracking-widest mb-2 relative z-10">{t('lbl_churn')}</h3>
                <p className="text-4xl font-black text-white relative z-10">{financialStats.churnRate}</p>
              </div>
            </div>
          </div>

          {/* SECCIÓN 2: GRÁFICAS BLINDADAS AL 100% */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            
            {/* Gráfica Ingresos (Sin ResponsiveContainer) */}
              ...
              <div className="w-full overflow-x-auto pb-4">
                <div style={{ width: '700px', height: '300px' }}>
                  {chartData && chartData.length > 0 ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-gray-500 text-xs font-bold uppercase tracking-widest">Esperando datos...</div>
                  )}
                </div>
              </div>
            </div>

            {/* Gráfica Suscriptores (Sin ResponsiveContainer) */}
            <div className="nm-inset p-6 sm:p-8 rounded-[2rem] border border-white/5 overflow-hidden">
              <h3 className="text-[11px] font-black text-gray-400 uppercase tracking-widest mb-6 flex items-center gap-2">
                  ...
                  {topFans.map((fan, index) => (
                    <div key={fan.id} className="flex items-center justify-between bg-black/60 backdrop-blur-md p-3 rounded-2xl border border-white/5 shadow-lg">
                      <div className="flex items-center gap-3">
                        <span className={`font-black text-lg ${index === 0 ? 'text-yellow-400 drop-shadow-[0_0_5px_rgba(250,204,21,0.8)]' : index === 1 ? 'text-gray-300' : 'text-orange-400'}`}>#{index + 1}</span>
