  ...
  useEffect(() => {
    ...
    const checkAndOpen = (hashToCheck: string) => {
      if (hashToCheck && hashToCheck.includes('-comment-')) {
        ...
        if (targetId && hasTargetInDescendants(comment.replies, String(targetId))) setShowReplies(true);
      }
    };
    ...
    <div id={`comment-${comment.id}`} className="flex flex-col mt-2 group/comment scroll-mt-40 transition-all duration-500">
      <div className="text-sm bg-white/5 p-3 rounded-xl border border-white/5 shadow-sm relative transition-all duration-500">
        ...
        <div className="flex items-center gap-3 mt-1.5 flex-wrap pl-8">
          ...
          {canDelete && <button onClick={() => onDelete(comment.id)} className="text-[11px] text-red-500 hover:text-red-400 font-bold opacity-100 lg:opacity-0 lg:group-hover/comment:opacity-100 transition-opacity duration-300">{t('btn_delete')}</button>}
          {!isCommentAuthor && (
            <>
              ...
              {(isPostOwner || isAdmin) && <button onClick={() => onBlock(comment.userId, comment.user?.username)} className="text-[11px] text-orange-500 hover:text-orange-400 font-bold opacity-100 lg:opacity-0 lg:group-hover/comment:opacity-100 transition-opacity duration-300">{t('btn_block')}</button>}
            </>
          )}
        </div>
      </div>
      {hasReplies && (
        <>
          <button onClick={() => setShowReplies(!showReplies)} className="text-[11px] text-gray-500 font-bold mt-2 ml-4 flex items-center gap-2 hover:text-gray-300 transition-colors">
            <div className="w-6 h-[1px] bg-gray-600"></div> {showReplies ? t('btn_hide_replies') : `${t('btn_view')} ${comment.replies.length} ${comment.replies.length === 1 ? t('lbl_reply') : t('lbl_replies')}`}
          </button>
          {showReplies && (
...
export default function Feed() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [
        ...
        postService.getAllPosts(), storyService.getFeedStories(),
    ...
    if (activeStory) {
      ...
      timer = setTimeout(() => {
        if (currentStoryIndex < userStories.length - 1) {
          ...
          setActiveStory(userStories[currentStoryIndex + 1]);
        } else {
          setActiveStory(null);
  ...
  const handleReact = async (postId: string, emoji: string) => {
    if (!user) return;
    try { await api.post(`/posts/${postId}/like`, { emoji }); fetchData(); } catch (error) {}
  };
  ...
  const handleMediaChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (hasOversizedFile) {
        setConfirmConfig({ title: "Archivo Muy Pesado", message: "¡Alto! El archivo es muy pesado (Máximo 500MB o 5 Minutos).", confirmText: "Entendido", hideCancel: true, onConfirm: () => setConfirmConfig(null) });
        if (fileInputRef.current) fileInputRef.current.value = '';
        return;
      }
  ...
  const openStory = async (story: any) => {
    setActiveStory(story); setCurrentStoryIndex(0); 
  ...
  const buildCommentTree = (comments: any[]) => {
    ...
    comments.forEach(c => {
      if (!c.parentId) { const rootNode = { ...c, replies: [] }; rootMap.set(c.id, rootNode); roots.push(rootNode); }
    });
    ...
    comments.forEach(c => {
      if (c.parentId) {
        ...
        if (rootId && rootMap.has(rootId)) rootMap.get(rootId).replies.push({ ...c, replies: [] });
      }
    });
    return roots;
  };
      ...
      <div className="min-h-screen bg-nm-base pb-24 sm:pb-10 relative">
        <nav className="sticky top-0 z-50 bg-[#0e0e0e]/90 border-b border-white/5 px-4 sm:px-6 py-3 flex justify-between items-center backdrop-blur-xl shadow-md">
          <div className="flex items-center">
            ...
            <h1 className="hidden md:block text-xl font-bold text-gray-200 tracking-wide">Feed</h1>
          </div>
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 custom-scrollbar px-4 sm:px-0 pt-2 sm:pt-0">
              ...
              {vipCreator && (
                <div onClick={() => router.push(`/${vipCreator.username}`)} className="flex flex-col items-center gap-1 cursor-pointer group shrink-0 relative mt-1.5">
                    ...
                    <span className="text-xs text-yellow-500 max-w-[64px] truncate font-bold mt-1 text-center">@{vipCreator.username || 'VIP Creador'}</span>
                </div>
              )}

              {stories.filter((story: any, index: number, self: any[]) => index === self.findIndex((s: any) => s.creator?.id === story.creator?.id)).map(story => (
                                ...
                                <div className="flex items-center gap-3">
                                  <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-3 py-1.5">
                                    {['❤️', '❤️‍🔥', '🫦', '🤤'].map((emoji) => {
                                      ...
                                      return (
                                        <button key={emoji} onClick={() => handleReact(post.id, emoji)} className={`flex items-center gap-1 transition-all duration-300 hover:scale-110 ${isSelected ? 'scale-110 opacity-100 grayscale-0 drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]' : 'opacity-40 grayscale hover:grayscale-0 hover:opacity-100'}`} title="Reaccionar">
                              ...
                              {commentingPostId === post.id && (
                                <div className="flex flex-col gap-2 animate-fade-in mt-2">
                                  ...
                                  <div className="flex gap-2">
                                    <input type="text" value={commentText} onChange={(e) => setCommentText(e.target.value)} placeholder={t('ph_write_comment')} className="flex-1 bg-black/50 border border-white/10 rounded-full px-5 py-2.5 text-sm text-white outline-none focus:border-blue-500/50" onKeyDown={(e) => e.key === 'Enter' && submitComment(post.id)}/>
                                    <button onClick={() => submitComment(post.id)} disabled={isSubmittingComment || !commentText.trim()} className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center text-white hover:bg-blue-500 transition-colors shrink-0 disabled:opacity-50"><Send className="w-4 h-4" /></button>
                                  </div>
                                </div>
                              )}
                              ...
                              {totalComments > 0 && (
                                 <div className="space-y-1 mt-4">
                                   {visibleComments.map((comment: any) => (
                                      <CommentNode key={comment.id} comment={comment} postId={post.id} postOwnerId={post.user?.id} currentUser={user} onReply={handleReplyClick} onDelete={handleDeleteComment} onReport={(commentId, username) => setReportData({ type: 'COMMENT', targetId: commentId, reportedUsername: username })} onBlock={handleBlockUser} isExpanded={isExpanded}/>
                                   ))}
                                   {totalComments > 3 && (
              ...
              {featuredBundle && (
                <div className="bg-gradient-to-br from-blue-900/40 to-purple-900/20 rounded-[2rem] border border-blue-500/30 shadow-2xl p-6 relative overflow-hidden group">
                  ...
                  <div className="flex flex-col gap-2 relative z-10">
                    <button onClick={(e) => { e.stopPropagation(); handleUnlockBundle(featuredBundle); }} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-2.5 rounded-xl transition-all shadow-[0_0_15px_rgba(59,130,246,0.5)] flex items-center justify-center gap-2 text-sm">
                      <Unlock className="w-4 h-4"/> {t('btn_buy_bundle')} ${featuredBundle.price}
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); router.push(`/${featuredBundle.creator?.username}`); }} className="w-full bg-transparent border border-blue-500/50 hover:bg-blue-500/10 text-blue-400 font-bold py-2.5 rounded-xl transition-all flex items-center justify-center gap-2 text-sm">
        ...
        {activeStory && (
          <div className="fixed inset-0 z-[99999] bg-black/95 flex flex-col animate-fade-in select-none pointer-events-auto">
            <div className="absolute top-4 left-0 right-0 z-[100001] flex gap-1 px-4 max-w-lg mx-auto w-full">
              {stories.filter((s: any) => s.creator?.id === activeStory.creator?.id).map((s: any, index: number) => (
                  <div key={s.id || index} className="h-1 flex-1 bg-white/30 rounded-full overflow-hidden shadow-sm">
                    <div className={"h-full bg-white transition-all duration-300 " + (index <= currentStoryIndex ? "w-full" : "w-0")}/>
                  </div>
                ))
              }
            </div>
            ...
            <div className="fixed top-0 left-0 w-full z-[100000] flex justify-between items-center p-4 bg-gradient-to-b from-black/80 to-transparent pt-8">
              ...
              <div className="flex items-center gap-3 z-[100001]">
                {(user?.id === activeStory.creator?.id || user?.role === 'ADMIN') && (
                  <button onClick={(e) => { e.stopPropagation(); handleDeleteStory(activeStory.id); }} className="w-10 h-10 bg-black/50 hover:bg-red-600 text-gray-300 hover:text-white rounded-full flex items-center justify-center backdrop-blur-md border border-white/10 transition-all shadow-lg cursor-pointer" title="Eliminar historia">
                    <Trash2 className="w-5 h-5" />
                  </button>
                )}
                <button onClick={(e) => { e.stopPropagation(); setActiveStory(null); }} className="w-10 h-10 bg-black/50 hover:bg-white/20 text-gray-300 hover:text-white rounded-full flex items-center justify-center backdrop-blur-md border border-white/10 transition-all shadow-lg cursor-pointer" title="Cerrar">
                  <X className="w-6 h-6" />
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 flex justify-center items-center p-4 pt-24 relative z-10 w-full h-full">
              <div className="absolute top-0 left-0 w-1/3 h-full z-20 cursor-pointer" onClick={(e) => {
                  e.stopPropagation();
                  const userStories = stories.filter((s: any) => s.creator?.id === activeStory.creator?.id);
              ...
              />
              <div className="absolute top-0 right-0 w-2/3 h-full z-20 cursor-pointer" onClick={(e) => {
                  e.stopPropagation();
                  const userStories = stories.filter((s: any) => s.creator?.id === activeStory.creator?.id);
                  if (currentStoryIndex < userStories.length - 1) {
        ...
        {isBoostModalOpen && (
          <BoostModal onClose={() => setIsBoostModalOpen(false)} creatorBalance={walletBalance} />
        )}
