...
export default function VerifyEmailPage() {
  ...
  useEffect(() => {
    if (token) {
      ...
      setStatus('ERROR');
      setMessage('No se encontró ningún token de verificación seguro.');
    }
  }, [token]);
    ...
    <div className="min-h-screen bg-black flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 mt-2 text-sm uppercase tracking-widest font-bold">Aduana de Seguridad</p>
        </div>
        ...
        <div className="glass-panel p-10 rounded-[2rem] border border-white/10 shadow-2xl bg-black/40 backdrop-blur-2xl text-center transition-all duration-500">
          ...
          {status === 'LOADING' && (
            <div className="flex flex-col items-center animate-fade-in">
              ...
              <h2 className="text-2xl font-black text-white mb-2">Comprobando Llave...</h2>
          ...
          {status === 'SUCCESS' && (
            <div className="flex flex-col items-center animate-fade-in">
              ...
              <h2 className="text-2xl font-black text-white mb-2">¡Acceso Concedido!</h2>
              ...
              >
                Iniciar Sesión Ahora <ArrowRight className="w-5 h-5" />
              </button>
            </div>
          )}
        ...
        <p className="text-center text-[10px] text-gray-500 mt-6 font-bold uppercase tracking-widest flex items-center justify-center gap-1">
          <ShieldCheck className="w-3 h-3" /> Verificación Criptográfica
        </p>
      </div>
    </div>
  );
