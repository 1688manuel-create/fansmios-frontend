...
export default function CouponsDashboard() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(error.response?.data?.error || 'Error al crear cupón');
    } finally { 
      ...
      <div className="min-h-screen bg-nm-base pb-24 relative overflow-hidden">
        ...
        <nav className="sticky top-0 z-40 bg-[#0a0a0a]/90 border-b border-white/5 px-6 py-4 flex justify-between items-center backdrop-blur-xl shadow-md">
          <h1 className="text-xl font-black text-white flex items-center gap-2">
            <Ticket className="w-6 h-6 text-pink-500" strokeWidth={2.5}/> Central de Ofertas
          </h1>
        ...
        <main className="max-w-6xl mx-auto mt-8 px-4 grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
          ...
          <div className="lg:col-span-1">
            <div className="nm-inset p-6 md:p-8 rounded-[2rem] border border-white/5 shadow-inner sticky top-28">
              <h2 className="text-xl font-black text-white mb-2">Crear Promoción</h2>
              <p className="text-gray-500 text-xs font-medium mb-6 leading-relaxed">
                Lanza códigos de descuento para atraer a nuevos fans o premiar a los más leales.
              </p>
              
              <form onSubmit={handleCreate} className="space-y-4">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest pl-2">Código Secreto</label>
                  <input type="text" value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} required placeholder="EJ: OFERTA50" className="w-full mt-1 bg-black/40 border border-white/5 nm-inset rounded-xl px-4 py-3 text-white outline-none focus:border-pink-500 font-black uppercase tracking-wider"/>
                </div>
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest pl-2">% de Descuento</label>
                  <input type="number" min="1" max="99" value={discountPercent} onChange={(e) => setDiscountPercent(e.target.value)} required placeholder="Ej: 20" className="w-full mt-1 bg-black/40 border border-white/5 nm-inset rounded-xl px-4 py-3 text-white outline-none focus:border-pink-500 font-bold"/>
                </div>
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest pl-2">Límite de Usos (Opcional)</label>
                  <div className="relative">
                    <Users className="w-4 h-4 text-gray-500 absolute left-4 top-1/2 -translate-y-1/2" />
                    <input type="number" min="1" value={maxUses} onChange={(e) => setMaxUses(e.target.value)} placeholder="Ilimitado" className="w-full mt-1 bg-black/40 border border-white/5 nm-inset rounded-xl pl-10 pr-4 py-3 text-white outline-none focus:border-pink-500"/>
                  </div>
                </div>
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest pl-2">Fecha de Caducidad (Opcional)</label>
                  <div className="relative">
                ...
                <button type="submit" disabled={isSubmitting} className="w-full bg-pink-600 hover:bg-pink-500 text-white font-black py-4 rounded-xl transition-all shadow-[0_0_20px_rgba(219,39,119,0.3)] mt-6 disabled:opacity-50 flex items-center justify-center gap-2">
                  {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin"/> : <><Plus className="w-5 h-5"/> Generar Cupón</>}
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {coupons.length === 0 ? (
              <div className="nm-inset p-12 rounded-[2rem] border border-white/5 text-center flex flex-col items-center justify-center min-h-[300px]">
                ...
                <h3 className="text-lg font-bold text-gray-300">Bóveda vacía</h3>
                <p className="text-gray-500 text-sm mt-2 max-w-sm">No has creado ningún cupón de descuento. Usa el panel izquierdo para lanzar tu primera oferta.</p>
              </div>
            ) : (
                ...
                {coupons.map(coupon => (
                  <div key={coupon.id} className={`p-5 rounded-2xl border transition-all flex flex-col justify-between ${coupon.active ? 'nm-inset border-pink-500/30 shadow-[inset_0_0_20px_rgba(219,39,119,0.05)]' : 'bg-black/50 border-white/5 opacity-60 grayscale'}`}>
                    ...
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <button onClick={() => copyToClipboard(coupon.code)} className="nm-btn p-2.5 rounded-lg text-gray-400 hover:text-white transition-colors" title="Copiar código">
                    ...
                    <div className="pt-4 border-t border-white/5 flex justify-between items-center">
                      <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded border ${coupon.active ? 'border-green-500/30 text-green-400 bg-green-500/10' : 'border-red-500/30 text-red-400 bg-red-500/10'}`}>
                        {coupon.active ? 'En Línea' : 'Apagado'}
                      </span>
                      ...
                      >
                        <Power className="w-3 h-3" /> {coupon.active ? 'Desactivar' : 'Activar'}
                      </button>
                    </div>
